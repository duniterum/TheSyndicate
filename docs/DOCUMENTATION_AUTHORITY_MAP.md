...
- `docs/VERIFIED_INTRODUCTION_V1_EXECUTION_BRIDGE.md` - operational execution bridge from direction-approved Verified Introduction V1 to non-activating implementation work. It defines phases, launch blockers, readbacks, Replit posture, and the founder priority order Verified Introduction -> SeatRecord721/ERC721 -> NFT/Archive. It has no deployment, publish, transaction, source activation, referral activation, public-source-link, alias route, source-dashboard, public source-aware-buy-path, or claim-UI authority.
