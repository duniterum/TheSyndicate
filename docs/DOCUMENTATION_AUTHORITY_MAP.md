...
- `docs/SOURCE_AWARE_LOCAL_TEST_PATH.md` - localhost-only source-aware test boundary for the internal PAUSED source record. It has no deployment, publish, production-route, transaction, source-record, source-activation, referral, registry-switch, public-source-link, public source-aware-buy-path, or claim-UI authority.
