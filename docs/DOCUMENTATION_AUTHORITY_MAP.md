...
- `docs/SOURCE_PUBLIC_PRODUCT_DECISION_FRAMEWORK.md` - operational decision framework for whether Source Attribution should become a public product and what a safe V1 could be. It recommends invite-only Verified Introduction V1 as MembershipSaleV3-only, manually approved, buyer-visible, buyer-clearable, direct-payout-first, and without aliases, claim UI, source dashboard, or product-wide attribution. It has no deployment, publish, transaction, source activation, referral activation, public-source-link, alias route, source-dashboard, public source-aware-buy-path, or claim-UI authority.
- `docs/SOURCE_PUBLIC_PRODUCT_FOUNDER_REVIEW_PACKET.md` - operational founder-review packet for approving, revising, rejecting, or deferring the Verified Introduction V1 posture. It converts the framework into founder decision options and has no deployment, publish, transaction, source activation, referral activation, public-source-link, alias route, source-dashboard, public source-aware-buy-path, or claim-UI authority.
