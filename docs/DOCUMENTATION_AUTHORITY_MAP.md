...
- `docs/SOURCE_REAL_CONDITION_TEST_PLAN.md` - exceptional founder prep plan for a possible before-July real-condition source-attributed $5 test, including proposed source-term update values, internal route gates, and readback sequence. It has no deployment, publish, transaction, source-terms-update, source-activation, referral, registry-switch, public-source-link, public source-aware-buy-path, or claim-UI authority.
- `docs/SOURCE_REAL_CONDITION_FOUNDER_CEREMONY_GUIDE.md` - founder-facing human ceremony guide for the exceptional real-condition source test. It explains step order, approval phrases, operator roles, and stop conditions. It has no deployment, publish, transaction, source-terms-update, source-activation, buy, re-pause, referral, registry-switch, public-source-link, public source-aware-buy-path, or claim-UI authority.
