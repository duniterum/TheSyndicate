...
| Public product decision | Product/legal/UX/security/release gates, `src/lib/public-product-decision-gate.ts`, `src/lib/source-public-product-framework.ts`, and `src/lib/verified-introduction-v1-execution.ts` | Users may be allowed to act only if separately approved. Verified Introduction V1 is approved as direction, not launch. | Backfilling activation from proof, memory, Chronicle, framework text, or direction approval alone. |
...
| 16 | Verified Introduction Current-Authority / Release QA Packet | Next: define latest-chain readbacks, Replit sync/publish gates, live QA, and stop conditions for any launch candidate. | Order 15. | Do not publish user-actionable controls without separate approval. |
| 17 | SeatRecord721 / ERC721 Identity | Next priority after Verified Introduction reaches safe implementation or launch-decision boundary. | Verified Introduction order 16/launch decision boundary. | Do not start identity work to avoid finishing Verified Introduction. |
| 18 | NFT / Archive Evolution | Priority after SeatRecord721 / ERC721 identity unless a verified dependency changes the order. | ERC721 identity work. | Do not make Archive/NFT source-aware by implication. |
| 19 | Whole-Protocol Continuous Excellence | Keep surfaces, docs, tests, status labels, and proof paths clean while referral matures. | Current GitHub truth. | Do not start unrelated future modules as fake-live placeholders. |
