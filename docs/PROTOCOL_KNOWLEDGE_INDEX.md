...
| Public source/referral product decision | DIRECTION APPROVED / LAUNCH NOT APPROVED. Verified Introduction V1 is approved as the next Source Attribution product direction, and the non-activating buyer skeleton now has an internal noindex review surface. Public source links, aliases, source dashboards, claim UI, and public source-aware buys still require a separate product/legal/UX/security/release/founder launch packet. | `src/lib/public-product-decision-gate.ts`, `src/lib/source-public-product-framework.ts`, `src/lib/verified-introduction-v1-execution.ts`, `src/lib/verified-introduction-v1-buyer-experience.ts`, `src/components/syndicate/VerifiedIntroductionBuyerExperience.tsx`, `src/routes/labs.verified-introduction-review.tsx`, `docs/SOURCE_PUBLIC_PRODUCT_DECISION_GATE.md`, `docs/SOURCE_PUBLIC_PRODUCT_DECISION_FRAMEWORK.md`, `docs/SOURCE_PUBLIC_PRODUCT_FOUNDER_REVIEW_PACKET.md`, `docs/VERIFIED_INTRODUCTION_V1_EXECUTION_BRIDGE.md` |
...
| Referral / Source Attribution | INACTIVE public product; one internal real-condition source-attributed V3 receipt has been validated under staged guardrails. Verified Introduction V1 is approved as direction only; non-activating buyer preview, clear-source, and failure-state infrastructure now exists behind tests and an internal review route. | `docs/REFERRAL_SOURCE_ATTRIBUTION_V1_READINESS.md`, `docs/SOURCE_ATTRIBUTION_CAPABILITY_MAP.md`, `docs/SOURCE_PUBLIC_PRODUCT_DECISION_FRAMEWORK.md`, `docs/VERIFIED_INTRODUCTION_V1_EXECUTION_BRIDGE.md`, source packet docs | `src/routes/referral.tsx`, `src/routes/labs.verified-introduction-review.tsx`, `src/lib/source-attributed-receipts.ts`, `src/lib/future-referral.ts`, `src/lib/source-real-condition-test.ts`, `src/lib/source-public-product-framework.ts`, `src/lib/verified-introduction-v1-execution.ts`, `src/lib/verified-introduction-v1-buyer-experience.ts`, `src/components/syndicate/VerifiedIntroductionBuyerExperience.tsx` | source receipt tests, source lifecycle tests, public product framework tests, Verified Introduction execution/buyer-experience tests, production coherence guards | Founder/operator review of the internal surface, then launch-packet or anti-abuse/disclosure hardening. | Claim UI, public links, aliases, source dashboard, product-wide attribution, or MLM-style language before launch approval. |
...
| 13 | Verified Introduction Launch-Packet Boundary | Next: founder/operator review, then decide whether to prepare launch-packet design, anti-abuse/disclosure hardening, or runtime integration plan. | Order 12. | Do not publish user-actionable controls without separate approval. |
| 14 | SeatRecord721 / ERC721 Identity | Next priority after Verified Introduction reaches safe implementation or launch-decision boundary. | Verified Introduction order 13/launch decision boundary. | Do not start identity work to avoid finishing Verified Introduction. |
| 15 | NFT / Archive Evolution | Priority after SeatRecord721 / ERC721 identity unless a verified dependency changes the order. | ERC721 identity work. | Do not make Archive/NFT source-aware by implication. |
| 16 | Whole-Protocol Continuous Excellence | Keep surfaces, docs, tests, status labels, and proof paths clean while referral matures. | Current GitHub truth. | Do not start unrelated future modules as fake-live placeholders. |
