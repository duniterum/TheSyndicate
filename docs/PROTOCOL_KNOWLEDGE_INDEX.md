...
| 15 | Verified Introduction Disclosure / Release Readback Hardening | Next: founder/operator review of the internal surface, launch packet draft, and anti-abuse model; then harden buyer disclosure/legal/accounting or current-authority/release QA gates. | Order 14. | Do not publish user-actionable controls without separate approval. |
| 16 | SeatRecord721 / ERC721 Identity | Next priority after Verified Introduction reaches safe implementation or launch-decision boundary. | Verified Introduction order 15/launch decision boundary. | Do not start identity work to avoid finishing Verified Introduction. |
| 17 | NFT / Archive Evolution | Priority after SeatRecord721 / ERC721 identity unless a verified dependency changes the order. | ERC721 identity work. | Do not make Archive/NFT source-aware by implication. |
| 18 | Whole-Protocol Continuous Excellence | Keep surfaces, docs, tests, status labels, and proof paths clean while referral matures. | Current GitHub truth. | Do not start unrelated future modules as fake-live placeholders. |
