...
| SourceRegistryV1 | DEPLOYED with one internal PAUSED source record. | `contracts/src/SourceRegistryV1.sol`, `docs/SOURCE_CREATION_CEREMONY_RUNBOOK.md` | `src/lib/source-policy-observability.ts`, `src/lib/source-registry-lifecycle.ts`, `src/routes/referral.tsx`, `src/routes/registry.tsx` | `contracts/test/SourceRegistryV1.t.sol`, source lifecycle tests, production coherence guards | Source activation readiness packet and current-authority preflight. | Treating source record existence as referral activation. |
...
3. Current owner/readback checks on Avalanche
4. Current source status and terms
5. Founder approval for the exact transaction
...
| --- | --- | --- | --- | --- |
| 1 | Source Activation Readiness Packet | Prepare exact non-transactional conditions for turning the existing PAUSED source ACTIVE later. | Current SourceRegistry owner/readback, source terms, window review. | Do not activate. |
