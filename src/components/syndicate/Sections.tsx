...
/* ─────────────────────── 03. Activity (Live + Demo split) ─────────────────────── */
type ActivityRow = {
  t: string;
  action: string;
  amount?: string;
  wallet: string;
  tag: "Vault" | "Member" | "Token" | "Liquidity" | "Founder";
  href?: string;
};

const LIVE_ACTIVITY: ActivityRow[] = [
  { t: "Genesis", action: "SYN ERC20 deployed on Avalanche C-Chain", amount: "1,000,000,000 SYN", wallet: "Contract",     tag: "Token",     href: avascanTokenUrl(SYN_ADDR) ?? undefined },
  { t: "Genesis", action: "Membership Distribution funded",          amount: "350,000,000 SYN",   wallet: "0x975a…Cec8",  tag: "Member",    href: avascanAddressUrl("0x975a4360FA808aC5D2Edb3c3412B2AeB9F5ECec8") ?? undefined },
  { t: "Genesis", action: "Vault Reserve funded",                    amount: "250,000,000 SYN",   wallet: "0x205D…f464",  tag: "Vault",     href: avascanAddressUrl("0x205DdC8921A4C60106930eE35e1F395c8D13f464") ?? undefined },
  { t: "Genesis", action: "Founder allocation funded",               amount: "120,000,000 SYN",   wallet: "0x88EC…Dd73",  tag: "Founder",   href: avascanAddressUrl("0x88EC79AF0d5A2F3b83022A1770c645506803Dd73") ?? undefined },
  { t: "Genesis", action: "Liquidity allocation funded",             amount: "100,000,000 SYN",   wallet: "0xa9b0…2e25",  tag: "Liquidity", href: avascanAddressUrl("0xa9b072db8DcDbb470235204B69D37275d74a2e25") ?? undefined },
  { t: "Genesis", action: "Partnerships allocation funded",          amount: "80,000,000 SYN",    wallet: "0xf5BE…caCf",  tag: "Token",     href: avascanAddressUrl("0xf5BEdEEfe48f746d96C1847a5595318579bBcaCf") ?? undefined },
  { t: "Genesis", action: "Contributors allocation funded",          amount: "50,000,000 SYN",    wallet: "0xa551…EF7a",  tag: "Token",     href: avascanAddressUrl("0xa55132346C70e63d0c4E0Fb15F35075760dDEF7a") ?? undefined },
  { t: "Genesis", action: "Future Ecosystem allocation funded",      amount: "50,000,000 SYN",    wallet: "0x2530…2963",  tag: "Token",     href: avascanAddressUrl("0x2530393881820AFe789f1c5D83817B70e46d2963") ?? undefined },
  { t: "Live",    action: "Membership Sale contract deployed",       amount: "USDC → SYN",        wallet: "0x0020…842d",  tag: "Member",    href: avascanAddressUrl("0x0020Df30C127306f0F5B44E6a6E4368D2855842d") ?? undefined },
  { t: "Live",    action: "Trader Joe SYN/USDC LP pool created",     amount: "200 SYN + 2 USDC",  wallet: "0xe124…9389",  tag: "Liquidity", href: txExplorerUrl("0x60f04521171a3f878f8b801c66a9e8c49f4931b9cb949b6c563b7ba47e9cbe05") },
];

const TAG_TONE: Record<string, "gold" | "navy" | "success" | "muted"> = {
  Vault: "gold", Member: "success", Token: "navy", Liquidity: "muted", Founder: "muted",
};

function ActivityTable({ rows, kind }: { rows: ActivityRow[]; kind: "live" | "demo" }) {
  return (
    <div className="surface overflow-hidden">
      <div className="grid grid-cols-12 gap-3 px-5 py-3 border-b border-border/60 mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
      ...
      <ul>
        {rows.map((row, i) => (
          <li key={i} className="grid grid-cols-12 gap-3 px-5 py-4 items-center text-sm border-b last:border-b-0 border-border/50">
            <div className="col-span-2 mono text-xs text-muted-foreground">{row.t}</div>
            <div className="col-span-5 flex items-center gap-3">
              <Pill tone={TAG_TONE[row.tag]}>{row.tag}</Pill>
              <span>{row.action}</span>
            </div>
            <div className="col-span-2 text-right mono text-xs">{row.amount}</div>
            <div className="col-span-2 mono text-xs text-muted-foreground">{row.wallet}</div>
            <div className="col-span-1 text-right">
              {kind === "live" && row.href ? (
                <a href={row.href} target="_blank" rel="noopener noreferrer" className="mono text-[10px] uppercase tracking-[0.2em] text-[var(--navy-soft)] hover:text-[var(--gold)]">↗</a>
              ) : (
                <span className="mono text-[9px] uppercase tracking-[0.18em] text-muted-foreground">demo</span>
              )}
            </div>
          </li>
        ))}
...
export function ActivityFeed() {
  return (
    <Section id="activity">
      <SectionHeader
        eyebrow="03 — Activity"
        title={<>Onchain activity, <span className="text-gradient-gold">honestly labeled.</span></>}
        description="Live events come from the deployed SYN token and the live Membership Sale contract on Avalanche. Pending modules stay out of the activity feed until they are verifiable."
      />
      <div className="mb-4 flex items-center gap-2">
        <Pill tone="success">LIVE ONCHAIN</Pill>
        <span className="text-xs text-muted-foreground">Initial mint transfers from the SYN deployment.</span>
      </div>
      <ActivityTable rows={LIVE_ACTIVITY} kind="live" />
    </Section>
  );
}
...
const QUICK_AMOUNTS = [5, 10, 25, 50, 75, 100, 250, 500, 1000];
const GROUP_ORDER: Array<RankTier["group"]> = ["Open Entry", "Active Members", "Deep Supporters", "High-Conviction"];
const GROUP_DESC: Record<RankTier["group"], string> = {
  "Open Entry": "Anyone can enter. The Archive opens at $5.",
  "Active Members": "Visible profile and stronger archive presence.",
  "Deep Supporters": "Higher status and deeper archive recognition.",
  "High-Conviction": "Manual review onboarding. Deepest archive recognition.",
};

export function JoinSection() {
  const [usdc, setUsdc] = useState(50);
  const safe = Math.max(0, usdc);
  const syn = safe / ACCESS_RATE_USDC_PER_SYN;
  const { current, next } = rankForUsdc(safe);
  const flow = vaultFlow(safe);
  const gap = next ? next.usdc - safe : 0;
  const pctToNext = next && current ? Math.min(100, ((safe - current.usdc) / (next.usdc - current.usdc)) * 100) : 100;
  const founderNo = 428 + Math.floor((safe / 50) % 7);
  const scoreMul = current?.scoreMultiplier ?? 1.0;
  const estScore = Math.round(40 + Math.log10(Math.max(safe, 1)) * 60 * scoreMul);

  const grouped = GROUP_ORDER.map((g) => ({ group: g, items: RANKS_V2.filter((r) => r.group === g) }));

  return (
    <Section id="join">
      <SectionHeader
        eyebrow="04 — Membership"
        title={<>Join from <span className="text-gradient-gold">$5 USDC</span></>}
        description="Open membership experiment. Type any USDC amount, see exactly what you get, how the Vault is reinforced, and which rank you unlock. Same rate for everyone — bigger amounts unlock status, not cheaper tokens."
      />

      {/* A. Fixed access rate card */}
      <div className="mb-6 grid gap-3 md:grid-cols-3">
        <div className="surface p-5 md:col-span-2 border-l-2 border-[var(--gold)]/60">
          <div className="mono text-[10px] uppercase tracking-[0.22em] text-[color:oklch(0.5_0.13_75)] mb-2">V1 Access Rate</div>
          <div className="text-2xl md:text-3xl font-semibold tracking-tight">
            <span className="mono">1 SYN</span> = <span className="text-gradient-gold mono">$0.01 USDC</span>
          </div>
          <p className="mt-3 text-sm text-muted-foreground">
            One rate for everyone. No discounts, no bonus tokens. Larger purchases unlock status, visibility, and archive recognition — never cheaper SYN.
          </p>
        </div>
        <div className="surface p-5 border-l-2 border-[var(--navy)]/40">
          <div className="mono text-[10px] uppercase tracking-[0.22em] text-muted-foreground mb-2">Utility membership</div>
          <p className="text-sm text-muted-foreground">
            SYN is not equity, debt, dividends, Vault ownership, or a promise of financial return.
          </p>
        </div>
      </div>

      {/* B. Custom amount simulator (main action) */}
      <GlassCard glow="gold" className="mb-6">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          <div className="lg:col-span-2 flex flex-col gap-4">
            <div className="mono text-[10px] uppercase tracking-[0.22em] text-[color:oklch(0.5_0.13_75)]">Custom purchase</div>
            ...
            {/* C. Quick amount buttons */}
            <div>
              <div className="mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground mb-2">Quick amounts</div>
              <div className="flex flex-wrap gap-1.5">
                {QUICK_AMOUNTS.map((v) => (
                  <button
                    key={v}
                    onClick={() => setUsdc(v)}
                    className={`mono text-[11px] uppercase tracking-[0.16em] rounded-full border px-3 py-1.5 transition-colors ${
                      usdc === v
                        ? "border-[var(--gold)] bg-[var(--gold)]/10 text-[color:oklch(0.5_0.13_75)]"
                ...
                <button
                  onClick={() => setUsdc(121)}
                  className="mono text-[11px] uppercase tracking-[0.16em] rounded-full border border-dashed border-border/80 px-3 py-1.5 text-muted-foreground hover:text-foreground"
                >
                  Custom
                </button>
              </div>
            ...
            <p className="text-xs text-muted-foreground">
              {safe < 5
                ...
                : `Fixed rate: 1 SYN = $0.01 USDC.`}
            </p>
          </div>

          <div className="lg:col-span-3 grid grid-cols-2 gap-3">
            <SimCard label="SYN received" value={`${Math.round(syn).toLocaleString("en-US")} SYN`} accent="gold" />
            <SimCard
              label="Rank unlocked"
              value={current ? current.name : "Below Citizen"}
              hint={current ? `×${current.scoreMultiplier.toFixed(2)} score` : "Min $5 to enter"}
              accent="navy"
            />
            <SimCard
              label={next ? "Next rank gap" : "Top tier"}
              value={next ? `${fmtUsd(gap)} → ${next.name}` : "Genesis Circle"}
              hint={next ? `${pctToNext.toFixed(0)}% to ${next.name}` : "Max rank"}
            />
            <SimCard label="Est. Founder #" value={`#${founderNo}`} hint="Permanent archive ID" />
            <div className="col-span-2 rounded-md border border-dashed border-border p-3">
              <div className="mono text-[9px] uppercase tracking-[0.2em] text-muted-foreground mb-2">
                Vault flow on {fmtUsd(safe)}
              </div>
              <div className="grid grid-cols-3 gap-2 text-center">
                <FlowSplit label="70% Vault" value={fmtUsd(flow.vault)} tone="gold" />
                <FlowSplit label="20% Liquidity" value={fmtUsd(flow.lp)} tone="navy" />
                <FlowSplit label="10% Operations" value={fmtUsd(flow.ops)} />
              </div>
            </div>
            <div className="rounded-md border border-emerald-500/30 bg-emerald-500/5 p-3 flex flex-col justify-center gap-2">
              <div className="flex items-center gap-2">
      ...
      {/* D. Tier cards grouped */}
      <div className="space-y-6">
        {grouped.map(({ group, items }) => (
          <div key={group}>
            <div className="flex items-baseline justify-between mb-3 flex-wrap gap-2">
              <div className="flex items-baseline gap-3">
                <Pill tone={group === "High-Conviction" ? "gold" : "muted"}>{group}</Pill>
                <span className="mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">{GROUP_DESC[group]}</span>
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {items.map((t) => {
                const f = vaultFlow(t.usdc);
                const idx = RANKS_V2.indexOf(t);
                const nextTier = RANKS_V2[idx + 1] ?? null;
                return (
                  <GlassCard key={t.name} className="flex flex-col gap-3">
                    <div className="flex items-center justify-between flex-wrap gap-1">
                      <Pill tone={t.manual ? "navy" : "muted"}>{t.name}</Pill>
                      {t.manual && <Pill tone="navy">Manual review</Pill>}
                    </div>
                    <div>
                      <div className="flex items-baseline gap-2">
                        <span className="mono text-3xl font-semibold tracking-tight">${t.usdc.toLocaleString("en-US")}</span>
                        <span className="mono text-xs text-muted-foreground">USDC</span>
                      </div>
                      <div className="mt-1 text-sm">
                        <span className="mono text-gradient-gold font-semibold">{t.syn.toLocaleString("en-US")} SYN</span>
                        <span className="text-muted-foreground"> · 1 SYN = $0.01</span>
                      </div>
                      <div className="mono mt-1 text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
                        Tier {String(RANKS_V2.indexOf(t) + 1).padStart(2, "0")} · {t.group}
                      </div>
                    </div>
                    ...
                    <div className="rounded-md border border-dashed border-border/70 p-2.5">
                      <div className="mono text-[9px] uppercase tracking-[0.2em] text-muted-foreground mb-1.5">Vault flow on ${t.usdc}</div>
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <FlowSplit label="70% Vault" value={fmtUsd(f.vault)} tone="gold" small />
                        <FlowSplit label="20% LP" value={fmtUsd(f.lp)} tone="navy" small />
                        <FlowSplit label="10% Ops" value={fmtUsd(f.ops)} small />
                      </div>
                    </div>
                    <div className="mt-auto pt-1 text-[11px] mono text-muted-foreground">
                      {nextTier ? `Next: ${nextTier.name} at $${nextTier.usdc.toLocaleString("en-US")}` : "Top tier"}
                    </div>
                    <CTAButton variant={t.manual ? "ghost" : "gold"} href="/join" className="w-full">
                      {t.manual ? "Manual Review — Apply" : `Buy ${t.name} · $${t.usdc}`}
                    </CTAButton>
                  </GlassCard>
                );
              })}
      ...
      {/* E. Honest risk box */}
      <div className="mt-8 rounded-lg border border-dashed border-border/80 p-5 bg-panel/40">
        <div className="mono text-[10px] uppercase tracking-[0.22em] text-[color:oklch(0.5_0.13_75)] mb-2">
          Public Experiment · No Promises
        </div>
        <p className="text-sm text-muted-foreground leading-relaxed">{LEGAL_DISCLAIMER}</p>
      </div>
    </Section>
  );
}

function SimCard({ label, value, hint, accent }: { label: string; value: string; hint?: string; accent?: "gold" | "navy" }) {
  const cls = accent === "gold" ? "text-gradient-gold" : accent === "navy" ? "text-[var(--navy-soft)]" : "";
  return (
    <div className="rounded-md border border-border/60 bg-panel/40 p-3">
      ...
      <div className={`mono text-lg md:text-xl font-semibold mt-1 ${cls}`}>{value}</div>
      {hint && <div className="mono text-[10px] text-muted-foreground mt-1">{hint}</div>}
    </div>
  );
}

function FlowSplit({ label, value, tone, small }: { label: string; value: string; tone?: "gold" | "navy"; small?: boolean }) {
  const cls = tone === "gold" ? "text-gradient-gold" : tone === "navy" ? "text-[var(--verify)]" : "";
  return (
    <div>
      <div className={`mono ${small ? "text-xs" : "text-sm"} font-semibold ${cls}`}>{value}</div>
...
const VERIFY_ENTITIES = [
  ...
  { label: "NFT Contract",       addr: SYNDICATE_CONFIG.NFT_CONTRACT_ADDRESS },
