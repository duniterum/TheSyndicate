...
**The discrepancy (truth bug, low risk):** `CockpitCollector.tsx` hardcodes ID 1 `supplyMode: "uncapped"` and renders the label **"Uncapped supply"**, and several docs (COCKPIT_WIDGET_INVENTORY, PRODUCT_MEMORY_AND_FUTURE_LOOPS, DEAD_COMPONENT_SALVAGE_REPORT) repeat "First Signal is uncapped". The deployed contract caps it at **10,000**. `archive-preview-catalog.ts` already agrees (`proposedMaxSupply: 10_000`).
