...
## Cockpit composition (index.tsx) — cockpit-forward first impression
- ProtocolHero is the COVER only (identity + radial engine + Protocol Overview). It is intentionally CONTENT-SIZED — NEVER re-add `min-h-[calc(100svh-64px)]` to its `<section>`/cover grid. **Why:** a full-screen hero buried the live cockpit (KPIs/engines/heartbeat) below the fold; Sprint 1B pulled it up.
- **HeroEntryStrip** (exported from ProtocolHero.tsx) owns the conversion buy box (EntryRail) + ChapterStrip and is mounted in index.tsx BELOW the cockpit (after HomeActivityTape, before "01 How it works"), NOT inside the cover. It owns its own amount/useQuoteSyn state. Don't move the buy box back into the hero cover.
- There is NO hero facts/metrics grid — that role is HomeKpiGrid's. The old `BottomFacts` block was removed because it duplicated HomeKpiGrid; don't re-add a metrics grid inside ProtocolHero.
- Hero cover mobile order (order utilities): identity+CTAs -> live proof metrics (HeroRight) -> economy visual (HeroEngine); `xl:` restores Left | Engine | Right.
- Order: curated ticker -> ProtocolHero (compact cover) -> HomeKpiGrid -> ProtocolEnginesPanel -> MilestoneApproachingTile -> LivePulseStrip -> HomeActivityTape -> HeroEntryStrip -> ActMarker "01 How it works" (...) -> ActMarker "02 Verify" (...) -> CTA Section -> RiskDisclaimer.
